    ...
    for i in block_prac:
        ...
        if len(order) >= nback_level:    #1-> correct; 2-> miss; 3-> incorrect
            if order[-1] == order[-2] and key_pressed[-1][0] == 'm':
                txt_feedback.text = 'That was right! This digit appeared just above.'
                txt_feedback.draw()
                win.flip()
                core.wait(3)
...
np.savetxt(file_name[:-4]+'_scores.csv',out_score, fmt='%f', delimiter=',',
    header='meancorr_RT,n-level,corr_rn1,corr_rn2,corr_rn3,corr_rn4,corr_rn5,corr_rn6,corr_rn7,corr_rn8,err_rn1,err_rn2,err_rn3,err_rn4,err_rn5,err_rn6,err_rn7,err_rn8,mss_rn1,mss_rn2,mss_rn3,mss_rn4,mss_rn5,mss_rn6,mss_rn7,mss_rn8')
