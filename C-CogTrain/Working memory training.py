...
for session_trial in range(20):
    ...
    txt_session_t.text = 'Block %s' % (day_session)
    ...
    while True:
        ...
        if 'escape' in k:
            ...
            tmp_meanscore = []      #stores all values for each day
            tmp_dayscore = []       #this stores the mean score per day
            tmp_date_reg = ['xxxx_xxx_xx'] #a list of days with data
            ...
            for index, i in data.iterrows():   #will iterate over each row
                ...
                else:   #creates new dates
                    tmp_dayscore.append(np.mean(tmp_meanscore))
                    tmp_meanscore = [float(tmp_level)]
    ...
    for active_trial in block:
        ...
        if active_trial['area'] == order[len(order)-(nback_level+1)]:   #if an n-back repetition occurs
            if len(key_pressed) > 0:
                responses.append([key_pressed[-1][1]])
            else:
                responses.append([2])   #penalisation
                errors += 1
        else:   #if keypress without n-back rep
            if len(key_pressed) > 0:
